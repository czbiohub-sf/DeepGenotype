...
- Per-sample alignment files organized by genotype outcome, located in `DeepGenotype_outputs/{Sample_ID}/alignments/consensus_seq_against_template/` (only present when consensus grouping is enabled):
  - Alignments from the error-corrected (consensus) genotyping pass. Similar read groups (within the edit distance threshold) are clustered together, and a consensus sequence is formed via weighted majority vote before genotyping. This produces fewer, cleaner entries with sequencing errors collapsed.
  - Contains one text file per observed genotype (e.g., `wt_allele.txt`, `HDR_perfect.txt`), with pairwise alignments showing each consensus sequence against its reference template.
